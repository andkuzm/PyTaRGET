...
class EncDecDataset(ATRDataset):
    ...
    def get_input(self, row, tokenizer):
        input = row["input"]
        return tokenizer.encode(input.replace("\t", "    "), return_tensors="pt")

    def get_output(self, row, tokenizer):
        output = row["output"]
        return tokenizer.encode(output.replace("\t", "    "), return_tensors="pt")
...
class PLBARTDataset(EncDecDataset):
    def get_input(self, row, tokenizer):
        ...
        return tokenizer.encode(input.replace("\t", "    "), add_special_tokens=False, return_tensors="pt")

    def get_output(self, row, tokenizer):
        output = "__python__" + row["output"] + tokenizer.eos_token
        return tokenizer.encode(output.replace("\t", "    "), add_special_tokens=False, return_tensors="pt")
...
class DecoderDataset(ATRDataset):
    ...
    def get_inference_input(self, row, tokenizer):
        return tokenizer.encode(row["input"].replace("\t", "    "), return_tensors="pt")
