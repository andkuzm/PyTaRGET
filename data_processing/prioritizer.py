...
class HunkPrioritizer:
    ...
    def create_changed_document(self, hunk):
        ...
        if "annotated_doc_seq" not in hunk:
            hunk["annotated_doc_seq"] = self.tokenizer.encode(hunk["annotated_doc"].replace("\t", "<TAB>"))
    ...
    def get_tfidf_sim(self, target, changes):
        ...
        tokenized_docs = [
            tokens for tokens in [self.tokenizer.encode(target.replace("\t", "<TAB>"))] + [c.get("annotated_doc_seq", []) for c in changes]
